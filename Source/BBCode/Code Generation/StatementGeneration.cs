...
public partial class CodeGeneratorForMain : CodeGenerator
{
    ...
    void GenerateCodeForLiteralString(string literal)
    {
        ...
        using (RegisterUsage.Auto reg = Registers.GetFree())
        {
            ...
            {
                AddInstruction(Opcode.Move, reg.Register.ToPtr(i * charSize, BitWidth._16), literal[i]);
            }

            AddInstruction(Opcode.Move, reg.Register.ToPtr(literal.Length * charSize, BitWidth._16), '\0');
        }
    ...
    void GenerateCodeForStatement(Field field)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < type.SizeBytes; i++)
            {
                AddComment($"{i}:");
    ...
    void GenerateCodeForValueSetter(Field statementToSet, StatementWithValue value)
    {
        ...
        if (prevType is PointerType pointerType)
        {
            ...
            GenerateCodeForStatement(value);
            HeapStore(StackTop - valueType.SizeBytes, fieldOffset);
    ...
    BBLangGeneratorResult GenerateCode(CompilerResult compilerResult, MainGeneratorSettings settings)
    {
        ...
        AddInstruction(Opcode.Pop32); // Pop abs global offset

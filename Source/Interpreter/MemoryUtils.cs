...
public static class MemoryUtils
{
    ...
    public static unsafe void SetData(this Span<byte> memory, int address, RuntimeValue data, BitWidth size)
    {
        ...
        {
            case BitWidth._8:
                memory.Set(address, data.Byte);
                break;
            case BitWidth._16:
                memory.Set(address, data.Char);
                break;
            case BitWidth._32:
                memory.Set(address, data.Int);
                break;
            default:
    ...
    public static unsafe void SetData(this byte[] memory, int address, RuntimeValue data, BitWidth size = BitWidth._32)
        => memory.AsSpan().SetData(address, data, size);
    public static unsafe RuntimeValue GetData(this byte[] memory, int address, BitWidth size = BitWidth._32)
        => memory.AsSpan().GetData(address, size);
}
